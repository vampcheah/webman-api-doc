...
use vampcheah\ApiDocWebman\lib\Tools;
...
class BootstrapApiDoc extends ApiDoc
{
  ...
  public function check_auth()
  {
    ...
    if (request()->method() == "POST") {
      ...
      $session->set(
        "api_doc_webman_check_login",
        md5($auth_password . "-" . date("Y/m"))
      );
  ...
  public function getHtml($type = \ReflectionMethod::IS_PUBLIC)
  {
    ...
    if (
      $hasSession != md5($config["auth"]["auth_password"] . "-" . date("Y/m"))
    ) {
      $checkSession = false;
    } else {
      $checkSession = true;
...
EXT;
  ...
  private function _getReturnData($data = [])
  {
    ...
    $html .=
      '<div class="table-item col-md-12"><p class="table-title"><span class="btn  btn-sm btn-success">返回参数</span></p>';
    $html .=
      '<table class="table"><tr><td>参数</td><td>类型</td><td>描述</td></tr>';
    foreach ($data as $v) {
      $html .=
        '<tr>
                        <td>' .
        Tools::getSubValue("return_name", $v, "") .
        '</td>
                        <td>' .
        Tools::getSubValue("return_type", $v, "") .
        '</td>
                        <td>' .
        Tools::getSubValue("return_title", $v, "") .
        '</td>
                      </tr>';
    }
    $html .= "</table></div>";
...
EXT;
  ...
  private function _getParamData($data = [])
  {
    ...
    $html .=
      '<div class="table-item col-md-12"><p class="table-title"><span class="btn  btn-sm btn-danger">请求参数</span></p>';
    $html .=
      '<table class="table"><tr><td>参数</td><td>类型</td><td>描述</td><td>默认值</td><td>是否必须</td></tr>';
    foreach ($data as $v) {
      $html .=
        '<tr>
                        <td>' .
        Tools::getSubValue("param_name", $v, "") .
        '</td>
                        <td>' .
        Tools::getSubValue("param_type", $v, "") .
        '</td>
                        <td>' .
        Tools::getSubValue("param_title", $v, "") .
        '</td>
                        <td>' .
        Tools::getSubValue("param_default", $v, "无默认值") .
        '</td>
                        <td>' .
        Tools::getSubValue("param_require", $v, "非必须") .
        '</td>
                      </tr>';
    }
    $html .= "</table></div>";
  ...
  private function _getHeaderData($data = [])
  {
    ...
    $html .=
      '<div class="table-item col-md-12"><p class="table-title"><span class="btn  btn-sm btn-primary">请求Header</span></p>';
    $html .=
      '<table class="table"><tr><td>参数</td><td>类型</td><td>描述</td><td>默认值</td><td>是否必须</td></tr>';
    foreach ($data as $v) {
      $html .=
        '<tr>
                        <td>' .
        Tools::getSubValue("header_name", $v, "") .
        '</td>
                        <td>' .
        Tools::getSubValue("header_type", $v, "") .
        '</td>
                        <td>' .
        Tools::getSubValue("header_title", $v, "") .
        '</td>
                        <td>' .
        Tools::getSubValue("header_default", $v, "无默认值") .
        '</td>
                        <td>' .
        Tools::getSubValue("header_require", $v, "非必须") .
        '</td>
                      </tr>';
    }
    $html .= "</table></div>";
  ...
  private function _getJsonData($data = [], $actionName = "")
  {
    ...
    $html .=
      '<div class="table-item col-md-12"><p class="table-title"><span class="btn btn-sm" style="background:#e030dc;color:#fff">返回Json</span></p>';
    $html .=
      '<table class="table"><tr><td>类型：JSON&emsp; <button id="collapse-btn' .
      $actionName .
      '" class="btn btn-sm" style="background:#fd5f9e;color:#fff">折叠</button>&nbsp;<button id="expand-btn' .
      $actionName .
      '"class="btn btn-sm" style="background:#fd5f9e;color:#fff">展开</button></td><td></td></tr>';
    foreach ($data as $key => $v) {
      $html .= '<tr style="display:none"><td colspan="2">';
      $html .=
        '<textarea class="form-control RawJson_' .
        $actionName .
        '" aria-label="With textarea" style="height:150px;">' .
        Tools::getSubValue("json_content", $v, "无数据") .
        "</textarea>";
      $html .= "</td></tr>";
      $html .=
        '<tr style="background:#1d1f21;"><td><div id="json_' .
        $actionName .
        '"></div></td></tr>';
      $html .=
        '<script>$(function() {
        ...
        '").JSONView(\'collapse\');
                      $("#json-collapsed' .
        $actionName .
        '").JSONView(json, {collapsed: true, nl2br: true});
                      $("#collapse-btn' .
        $actionName .
        '").on(\'click\', function() {
                        $("#json_' .
  ...
  private function _getCodeData($data = [])
  {
    ...
    $html .=
      '<div class="table-item col-md-12"><p class="table-title"><span class="btn  btn-sm btn-warning">状态码说明</span></p>';
    $html .= '<table class="table"><tr><td>状态码</td><td>描述</td></tr>';
    foreach ($data as $v) {
      $html .=
        '<tr>
                        <td>' .
        Tools::getSubValue("code", $v, "") .
        '</td>
                        <td>' .
        Tools::getSubValue("content", $v, "暂无说明") .
        '</td>
                      </tr>';
    }
    $html .= "</table></div>";
  ...
  private function _getActionItem($className, $actionName, $actionItem)
  {
    $html = <<<EXT
      ...
      Tools::getSubValue("header", $actionItem, [])
                    ...
                    {$this->_getParamData(
      Tools::getSubValue("param", $actionItem, [])
    )}
                    {$this->_getReturnData(
      Tools::getSubValue("return", $actionItem, [])
    )}
                    {$this->_getJsonData(
      Tools::getSubValue("json", $actionItem, []),
      $className . "_" . $actionName
    )}
                    {$this->_getCodeData(
      Tools::getSubValue("code", $actionItem, [])
    )}
                </div>
EXT;
...
EXT;
  ...
  private function _getJs()
  {
    ...
    $js .=
      '<script src="https://cdn.bootcss.com/twitter-bootstrap/4.4.1/js/bootstrap.min.js" type="text/javascript"></script>';
    $js .=
      '<script src="https://cdn.bootcdn.net/ajax/libs/jquery-jsonview/1.2.2/jquery.jsonview.min.js"></script>';
